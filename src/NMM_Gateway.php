...
class NMM_Gateway extends WC_Payment_Gateway {
    ...
    private function get_qr_code($crypto, $walletAddress, $cryptoTotal, $order->get_id()) {        
        ...
        try {
            QRcode::png($qrData, $dirWrite . 'tmp_qrcode_' . $orderId . '.png', QR_ECLEVEL_H);            
        }
        catch (\Exception $e) {
     ...
     $fileName = 'tmp_qrcode_' . $orderId . '.png';
    }
    ...
    private function output_thank_you_html($crypto, $orderWalletAddress, $cryptoTotal, $orderId) {        
        ...
        $qrCode = $this->get_qr_code($crypto, $orderWalletAddress, $cryptoTotal, $order->get_id());        
