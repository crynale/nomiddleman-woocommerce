...
class NMM_Cryptocurrencies {
	...
	public static function is_valid_wallet_address($cryptoId, $address) {
        ...
        if ($cryptoId === 'BTC') {            
            return preg_match('/^[13][a-km-zA-HJ-NP-Z0-9]{24,42}|bc[a-z0-9]{8,87}/', (string) $address);
        }
        if ($cryptoId === 'ETH') {
            return preg_match('/0x[a-fA-F0-9]{40,42}/', (string) $address);
        }
        if ($cryptoId === 'XMR') {
            // return preg_match('/4[0-9AB][1-9A-HJ-NP-Za-km-z]{93}/', $address);

            // 2-15-2019 not testing Monero
            return strlen((string) $address) > 30;
        }
        if ($cryptoId === 'DOGE'){
            return preg_match('/^D{1}[5-9A-HJ-NP-U]{1}[1-9A-HJ-NP-Za-km-z]{32}/', (string) $address);
        }
        if ($cryptoId === 'LTC') {
            return preg_match('/^[LM3][a-km-zA-HJ-NP-Z1-9]{26,33}|l[a-z0-9]{8,87}/', (string) $address);
        }
        if ($cryptoId === 'ZEC') {
            $isTAddr =  preg_match('/^t1[a-zA-Z0-9]{33,36}/', (string) $address);
            $isZAddr = preg_match('/^z[a-zA-Z0-9]{90,96}/', (string) $address);

            return $isTAddr || $isZAddr;
        }
        if ($cryptoId === 'BCH') {
            $isOldAddress = preg_match('/^[13][a-km-zA-HJ-NP-Z1-9]{25,42}/', (string) $address);
            $isNewAddress1 = preg_match('/^(q|p)[a-z0-9]{41}/', (string) $address);
            $isNewAddress2 = preg_match('/^(Q|P)[A-Z0-9]{41}/', (string) $address);

            return $isOldAddress || $isNewAddress1 || $isNewAddress2;
        }
        if ($cryptoId === 'DASH') {
            return preg_match('/^X[1-9A-HJ-NP-Za-km-z]{33}/', (string) $address);
        }
	if ($cryptoId === 'HTH') {
            return preg_match('/^X[1-9A-HJ-NP-Za-km-z]{33}/', (string) $address);
        }	
        if ($cryptoId === 'XRP') {
            return preg_match('/^r[0-9a-zA-Z]{28,37}/', (string) $address);
        }
        if ($cryptoId === 'ONION') {
            return preg_match('/^D[0-9a-zA-Z]{33}/', (string) $address);
        }
        if ($cryptoId === 'BLK') {
            return preg_match('/^B[0-9a-zA-Z]{32,36}/', (string) $address);
        }
        if ($cryptoId === 'VRC') {
            return preg_match('/^V[0-9a-zA-Z]{32,36}/', (string) $address);
        }
        if ($cryptoId === 'ETC') {
            return preg_match('/^0x[a-fA-F0-9]{40,42}/', (string) $address);
        }
        ...
        if ($cryptoId === 'BTG') {
            return preg_match('/^[AG][a-km-zA-HJ-NP-Z0-9]{26,42}|bt[a-z0-9]{8,87}/', (string) $address);
        }
        if ($cryptoId === 'EOS') {
            return strlen((string) $address) >= 1 && strlen((string) $address) <= 12;
        }
        if ($cryptoId === 'BSV') {
            return preg_match('/^[13][a-km-zA-HJ-NP-Z0-9]{26,42}|q[a-z0-9]{9,88}/', (string) $address);
        }
        if ($cryptoId === 'VET') {
            return preg_match('/^0x[a-fA-F0-9]{40,42}/', (string) $address);
        }
        if ($cryptoId === 'TRX') {
            return preg_match('/^T[a-km-zA-HJ-NP-Z0-9]{26,42}/', (string) $address);
        }
        if ($cryptoId === 'XLM') {
            return preg_match('/^G[A-Z0-9]{55}/', (string) $address);
        }
        if ($cryptoId === 'QTUM') {
            return preg_match('/^Q[0-9a-zA-Z]{31,35}/', (string) $address);
        }
        if ($cryptoId === 'ADA') {
            $match1 = preg_match('/^Ddz[0-9a-zA-Z]{80,120}/', (string) $address);
            $match2 = preg_match('/^Ae2tdPwUPE[0-9a-zA-Z]{46,53}/', (string) $address);
            $match3 = preg_match('/^addr[0-9a-zA-Z]{99}/', (string) $address);

            return $match1 || $match2 || $match3;
        }
        if ($cryptoId === 'XTZ') {
            return preg_match('/^tz1[0-9a-zA-Z]{30,39}/', (string) $address);
        }
        if ($cryptoId === 'MLN') {
            return preg_match('/^0x[a-fA-F0-9]{40,42}/', (string) $address);
        }
        ...
        if ($cryptoId === 'ONT') {
            return preg_match('/^A[0-9a-zA-Z]{31,35}/', (string) $address);
        }
        if ($cryptoId === 'BAT') {
            return preg_match('/^0x[a-fA-F0-9]{40,42}/', (string) $address);
        }
        if ($cryptoId === 'BCD') {
            return preg_match('/^1[0-9a-zA-Z]{31,35}/', (string) $address);
        }
        if ($cryptoId === 'BCN') {
            return preg_match('/^2[0-9a-zA-Z]{91,99}/', (string) $address);
        }
        if ($cryptoId === 'BNB') {
            return preg_match('/^bnb[a-zA-Z0-9]{37,48}/', (string) $address) || preg_match('/^0x[a-fA-F0-9]{40,42}/', (string) $address);
        }
        if ($cryptoId === 'DCR') {
            return preg_match('/^D[0-9a-zA-Z]{31,35}/', (string) $address);
        }
        if ($cryptoId === 'DGB') {
            return preg_match('/^D[0-9a-zA-Z]{31,35}/', (string) $address);
        }
        ...
        if ($cryptoId === 'LSK') {
            return preg_match('/^[0-9a-zA-Z]{17,22}L/', (string) $address);
        }
        if ($cryptoId === 'MIOTA') {
            return preg_match('/^[0-9a-zA-Z]{85,95}/', (string) $address);
        }
        if ($cryptoId === 'MKR') {
            return preg_match('/^0x[a-fA-F0-9]{40,42}/', (string) $address);
        }
        ...
        if ($cryptoId === 'POT') {
            return preg_match('/^P[0-9a-zA-Z]{31,35}/', (string) $address);
        }
        if ($cryptoId === 'WAVES') {
            return preg_match('/^3[0-9a-zA-Z]{31,35}/', (string) $address);
        }
        if ($cryptoId === 'XEM') {
            return preg_match('/^N[0-9a-zA-Z]{35,45}/', (string) $address);
        }
        if ($cryptoId === 'ZRX') {
            return preg_match('/^0x[a-fA-F0-9]{40,42}/', (string) $address);
        }
        ...
        if ($cryptoId === 'XMY') {
            return preg_match('/^[M45][a-zA-Z0-9]{31,38}/', (string) $address);   
        }
        if ($cryptoId === 'BTX') {
            return preg_match('/^[2s][a-km-zA-HJ-NP-Z0-9]{24,42}|btx[a-z0-9]{8,87}/', (string) $address);
        }
        if ($cryptoId === 'SAFE') {
            return preg_match('/^R[a-zA-Z0-9]{33,36}/', (string) $address);
        }
        if ($cryptoId === 'GRS') {
            return preg_match('/^[F3][a-km-zA-HJ-NP-Z0-9]{24,42}/', (string) $address);
        }
        if ($cryptoId === 'APL') {
            return preg_match('/^APL-[a-zA-Z0-9-]{8,42}/', (string) $address);
        }
        if ($cryptoId === 'USDC') {
            return preg_match('/^0x[a-fA-F0-9]{40,42}/', (string) $address);
        }
        if ($cryptoId === 'NANO') {
            return preg_match('/^nano_[13][13456789abcdefghijkmnopqrstuwxyz]{59}/', (string) $address);
        }
